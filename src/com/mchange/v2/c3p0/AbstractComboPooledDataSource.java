...
/*
import curator package for zookeeper
 */
...
//          new Exception("*** NULL SETTER ***").printStackTrace();
    ...
    private static Logger log = Logger.getLogger(AbstractComboPooledDataSource.class);

    public static CuratorFramework createClient(String connectString, RetryPolicy retryPolicy, int connectionTimeoutMs, int sessionTimeMs) {
        CuratorFramework zkClient = null;
        log.info("creating zkClient...");
        try {
            zkClient = CuratorFrameworkFactory.builder().connectString(connectString)
                    ...
                    .build();
            log.info("zkClient created...");
        } catch (Exception e) {
            log.error("zkClient created failed...");
            e.printStackTrace();
        }
        return zkClient;
    }

    public static String getPathData(CuratorFramework zkClient, String dataPath) {
        String dataString = null;
        log.info("get data from Path:" + dataPath);
        try {
            if (zkClient.checkExists().forPath(dataPath) != null) {
                byte[] data = zkClient.getData().forPath(dataPath);
                dataString = new String(data);
                log.info("the data from Path:" + dataPath + " is " + dataString);
            } else {
                log.info("Path:" + dataPath + "is no existed...");
            }
        } catch (Exception e) {
            log.error("get data from Path:" + dataPath + " failed...");
            e.printStackTrace();
        }
        return dataString;
    ...
    public synchronized  void init() {
        ...
        CuratorFramework zkClient = createClient(connectString, new ExponentialBackoffRetry(1000, 3), 3000, 3000);
        zkClient.start();
        ...
        try {
            ...
            isConnected = false;
            log.error("zkclient init failed, check the ip&port is correct or the zk server is running...");
            e.printStackTrace();
        }

        if(isConnected == true) {
            String zkPath = rootPath +"/" +  appname +  "/";
            String urlPath = zkPath + "url";
            String driverPath = zkPath + "driver";
            String usernamePath = zkPath + "username";
            String passwdPath = zkPath + "passwd";
            String url = getPathData(zkClient, urlPath);
            String driver = getPathData(zkClient, driverPath);
            String username = getPathData(zkClient, usernamePath);
            String passwd = getPathData(zkClient, passwdPath);
            if (url != null && url.trim().length() > 0) {
                this.setJdbcUrl(url);

            } else {
                log.error("get url data failed, data is null...");
            }
            if (driver != null && driver.trim().length() > 0) {
                try {
                    this.setDriverClass(driver);
                } catch (PropertyVetoException e) {
                    log.error("set driver name error...");
                    e.printStackTrace();
                }

            } else {
                log.error("get driver data failed, data is null...");
            }
            if (username != null && username.trim().length() > 0) {
                this.setUser(username);

            } else {
                log.error("get username data failed, data is null...");
            }
            if (passwd != null && passwd.trim().length() > 0) {
                this.setPassword(passwd);

            } else {
                log.error("get password data failed, data is null...");
            }
        }

        else {
            log.error("fetch data from zookeeper:" + connectString + "failed...");
        }
