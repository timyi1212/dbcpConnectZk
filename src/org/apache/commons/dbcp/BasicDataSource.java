...
/*    public String getUuid() {
    ...
    private static Logger logger = Logger.getLogger(BasicDataSource.class);
    ...
    public static CuratorFramework createClient(String connectString, RetryPolicy retryPolicy, int connectionTimeoutMs, int sessionTimeMs) {
        ...
        try {
            zkClient = CuratorFrameworkFactory.builder().connectString(connectString)
                    ...
                    .build();
            logger.info("zkClient created...");
        } catch (Exception e) {
            logger.error("zkClient created failed...");
            e.printStackTrace();
        }
        return zkClient;
    }
    ...
    public static String getPathData(CuratorFramework zkClient, String dataPath) {
        ...
        logger.info("get data from Path:" + dataPath);
        try {
            if (zkClient.checkExists().forPath(dataPath) != null) {
                byte[] data = zkClient.getData().forPath(dataPath);
                dataString = new String(data);
                logger.info("the data from Path:" + dataPath + " is " + dataString);
            } else {
                logger.info("Path:" + dataPath + "is no existed...");
            }
        } catch (Exception e) {
            logger.error("get data from Path:" + dataPath + " failed...");
            e.printStackTrace();
        }
        return dataString;
   ...
   public synchronized  void init() {
        ...
        try {
            ...
            isConnected = false;
            logger.error("zkclient init failed, check the ip&port is correct or the zk server is running...");
            e.printStackTrace();
        }
        ...
        if(isConnected == true) {
            ...
            if (url != null && url.trim().length() > 0) {
                ...
                logger.error("get url data failed, data is null...");
            }
            if (driver != null && driver.trim().length() > 0) {
                this.driverClassName = driver;
            } else {
                logger.error("get driver data failed, data is null...");
            }
            if (username != null && username.trim().length() > 0) {
                this.username = username;
            } else {
                logger.error("get username data failed, data is null...");
            }
            if (passwd != null && passwd.trim().length() > 0) {
                this.password = passwd;
            } else {
                logger.error("get password data failed, data is null...");
            }
        }

        else {
            logger.error("fetch data from zookeeper:" + connectString + "failed...");
        }
